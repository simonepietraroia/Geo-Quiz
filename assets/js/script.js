...
/**
 * This function checks the answer after reciving the users choice rom the displayQuestion function.
 * It also increases the score after every question if correct
 * it also determins whether theere are any more questions
 */
function checkAnswer(event) {
    let selectedOption = event.target;
	let question = questions[currentQuestion];
	if (question.answer === parseInt(selectedOption.id.slice(-1))) {
		score++;
		scoreElem.innerText = score;
	}
	currentQuestion++;
	if (currentQuestion === questions.length) {
		alert(" Congradulations! Your score is " + score);
	} else {
		displayQuestion();
	}
}
